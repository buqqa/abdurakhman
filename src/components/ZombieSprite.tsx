...
export function ZombieSprite({ zombie }: { zombie: Zombie }) {
  ...
  return (
    <div className={`map-zombie ${zombie.facingLeft ? 'map-zombie--left' : ''} ${zombie.isSprinter ? 'map-zombie--sprinter' : ''} ${wasHit ? 'map-zombie--hit' : ''} ${isSpawning ? 'map-zombie--spawning' : ''}`} style={{ transform: `translate(${zombie.x - 15}px, ${zombie.y - 18}px)` }}>
