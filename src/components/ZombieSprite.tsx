...
export function ZombieSprite({ zombie }: { zombie: Zombie }) {
  ...
  const isSpawning = performance.now() - zombie.spawnedAt < (zombie.isBoss ? 1600 : 800);
  return (
    <div className={`map-zombie ${zombie.facingLeft ? 'map-zombie--left' : ''} ${zombie.isBoss ? 'map-zombie--boss' : ''} ${zombie.isSprinter ? 'map-zombie--sprinter' : ''} ${wasHit ? 'map-zombie--hit' : ''} ${isSpawning ? 'map-zombie--spawning' : ''}`} style={{ transform: `translate(${zombie.x - 15}px, ${zombie.y - 18}px)` }}>
