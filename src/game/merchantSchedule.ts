...
import { MAP_HEIGHT } from './mapConfig';
...
export function createMerchantVisits(difficulty: string): MerchantVisit[] {
  ...
  return visitRanges(difficulty).map(([from, to]) => {
    let position = { x: 315, y: 680 };
    for (let attempt = 0; attempt < 200; attempt += 1) {
      const candidate = { x: 55 + Math.random() * 475, y: 55 + Math.random() * (MAP_HEIGHT - 110) };
    ...
    const visit = { day: randomDay(from, to), ...position };
    visits.push(visit);
    return visit;
  });
}
