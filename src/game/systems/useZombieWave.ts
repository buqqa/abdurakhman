  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      ...
      const next = zombiesRef.current.map((zombie) => {
        ...
        const targetsPlayer = zombie.isBoss ? playerDistance <= 240 : playerDistance < baseDistance;
        const target = targetsPlayer ? player : base;
        const distance = targetsPlayer ? playerDistance : baseDistance;
        const attackDistance = targetsPlayer ? (zombie.isBoss ? 22 : 20) : (zombie.isBoss ? 76 : 68);
